...
const EditSubCategory = ({ close, data,fetchData }) => {
    ...
    // console.log("allCategory subcat page",allCategory)
                        ...
                        <div className='border focus-within:border-primary-200 rounded'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {
                                    subCategoryData.category.map((catId) => {
                                        const categoryObj = allCategory.find(cat => cat._id === catId)
                                        return (
                                            <p key={catId + "selectedValue"} className="bg-white shadow-md px-1 m-1 flex items-center gap-2">
                                                {categoryObj?.name || "Unknown"}
                                                <div className="cursor-pointer hover:text-red-600" onClick={() => handleRemoveCategorySelected(catId)}>
                                                    <IoClose size={20} />
                                                </div>
                                            </p>
                                        );
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    if (!subCategoryData.category.includes(value)) {
                                        setSubCategoryData((preve) => {
                                            return {
                                                ...preve,
                                                category: [...preve.category, value]  // push only _id
                                            };
                                        });
                                    }
                                }}
